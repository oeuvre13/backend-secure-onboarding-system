...
-- File: database_setup_DOCKER.sql
-- This script is designed to be run by Docker's entrypoint (e.g., in /docker-entrypoint-initdb.d/)
-- It assumes the 'customer_registration' database is already created by the Docker entrypoint.

-- IMPORTANT: The DROP DATABASE, CREATE DATABASE, and \c commands have been removed.
-- These operations are handled by the Docker PostgreSQL entrypoint itself.
-- This script should only contain commands to create tables, indexes, triggers, and insert data.

-- Drop existing tables to recreate fresh (important for clean re-initialization)
...
-- Ini harus didefinisikan SEBELUM trigger yang menggunakannya
